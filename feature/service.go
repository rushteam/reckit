...
type FeatureStore interface {
	...
	SetUserFeatures(ctx context.Context, userID int64, features map[string]float64, ttl time.Duration) error

	// SetItemFeatures 写入物品特征（可选，用于特征更新）
	SetItemFeatures(ctx context.Context, itemID int64, features map[string]float64, ttl time.Duration) error
}
...
type FeatureCache interface {
	...
	GetItemFeatures(ctx context.Context, itemID int64) (map[string]float64, bool)

	// SetItemFeatures 设置物品特征缓存
	SetItemFeatures(ctx context.Context, itemID int64, features map[string]float64, ttl time.Duration)

	// InvalidateUserFeatures 失效用户特征缓存
	InvalidateUserFeatures(ctx context.Context, userID int64)

	// InvalidateItemFeatures 失效物品特征缓存
	InvalidateItemFeatures(ctx context.Context, itemID int64)
