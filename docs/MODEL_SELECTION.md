...
| 召回 | Word2Vec | 文本/序列向量化 | 文本推荐、I2I召回 | 语义相似：捕捉文本/序列的语义相似性，适合文本丰富的场景 | 需要预训练词向量，OOV处理复杂 | 文本/序列 | 相似度分数或向量 | PyTorch (训练) + Golang (推理) |
| 召回 | BERT | 深度语义理解 | 文本语义召回、搜索推荐 | 语义理解强：捕捉深层语义，适合复杂文本场景 | 计算开销大：需要 RPC 调用外部服务，延迟较高 | 文本 | 语义向量 | PyTorch/TensorFlow (训练) + TorchServe/TF Serving (推理) |
